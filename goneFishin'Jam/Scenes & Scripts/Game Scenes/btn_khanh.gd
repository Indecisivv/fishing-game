...
func _on_btn_khanh_entered() -> void:
	if (self.disabled):
		return
	...
	btn_line_boiler.get_animation('lineboil').track_set_enabled(1, false)
...
func _on_btn_khanh_exited() -> void:
	...
	btn_line_boiler.get_animation('lineboil').track_set_enabled(1, true)
